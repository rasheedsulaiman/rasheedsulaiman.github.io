...
LinearAd.prototype.skipAd = function() {
  ...
  if (this._videoSlot && this._videoSlot instanceof HTMLVideoElement) {
      console.log('Attempting to stop video on mobile');

      try {
          // Pause the video
          this._videoSlot.pause();

          // Move to the end to try and stop playback
          this._videoSlot.currentTime = this._videoSlot.duration;

          // Forcefully stop the video
          this._videoSlot.removeAttribute('src'); // Remove the source to stop playback
          this._videoSlot.load(); // Reload to clear any buffered media
      } catch (error) {
          console.error('Error while stopping the video:', error);
      }
  } else {
      console.error('Video slot is not properly initialized or is not a video element');
  }

  // Emit AdStopped event after a slight delay
  var that = this;
  setTimeout(function() {
      that.onAdStopped();
